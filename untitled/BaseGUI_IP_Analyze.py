...
class Application(tk.Tk):
    ...
    def PDU_sniff(self):
        ...
        if sniffNum > 0:
            sniff(prn=(lambda x: self.ether_monitor_callback(x)), filter = 'ip dst 192.168.1.6',stop_filter=(lambda x: self.sniffFlag), count=sniffNum)
        elif sniffNum == 0:
            sniff(prn=(lambda x: self.ether_monitor_callback(x)), filter = 'ip dst 192.168.1.6',stop_filter=(lambda x: self.sniffFlag), count=sniffNum)
        else:
    ...
    def choosedPDUAnalysis(self, event):
        ...
        if temp == change_count_check :
            self.PDUAnalysisText.insert('end', '检验和正确\n')
        else:
            self.PDUAnalysisText.insert('end', '检验和错误\n')
        ...
        self.PDUAnalysisText.insert('end', '自动校验和：%04x(%s)\n' %(temp,str(temp)))
        self.PDUAnalysisText.insert('end', '计算校验和：%04x(%s)\n' %(change_count_check,str(change_count_check)))
        self.PDUAnalysisText.insert('end', '上层协议类型：' + str(choosedPacket[Ether].type) + '\n')
